...
function rewardAmount(): number {
  return Number(process.env.DAILY_LOGIN_REWARD_AMOUNT ?? '2');
}
...
export default async function handler(req: { body?: Input }) {
  ...
  const day = dateKey(payload.date);
  const referenceKey = `daily_login:${payload.user_id}:${day}`;
